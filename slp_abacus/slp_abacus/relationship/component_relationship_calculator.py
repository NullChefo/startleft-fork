...
class ComponentRelationshipCalculator:
    ...
    def __init__(self, abacus_otm: AbacusOTM):
        self.abacus_otm = abacus_otm

    def get_relationship(self, component_from: AbacusComponent, component_to: AbacusComponent) \
    ...
    def are_related(self, first: AbacusComponent, second: AbacusComponent) -> bool:
        """
    ...
    def __is_ancestor(self, component: AbacusComponent, ancestor: AbacusComponent) -> bool:
        return component.parent_type == ParentType.COMPONENT and \
               (component.parent == ancestor.id
                or self.__is_ancestor(self.abacus_otm.get_component_by_id(component.parent), ancestor))

    def __is_ancestor_of_any_clone(self, component: AbacusComponent, ancestor: AbacusComponent) -> bool:
        if not component.clones_ids:
            return False

        for clone_id in component.clones_ids:
            if self.__is_ancestor(self.abacus_otm.get_component_by_id(clone_id), ancestor):
                return True
