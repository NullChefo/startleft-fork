...
def _merge_component_configurations(otm_components: List[AbacusComponent]) -> Dict:
    merge_configuration = {}
...
def _build_singleton_component(otm_components: List[AbacusComponent]) -> AbacusComponent:
...
class SingletonTransformer(Transformer):
    ...
    def __init__(self, otm: AbacusOTM):
        super().__init__(otm)
        self.otm_components = self.otm.components
        self.otm_dataflows = self.otm.dataflows

        self.singleton_component_relations: Dict[str, AbacusComponent] = {}
    ...
    def __is_not_parent(self, component: AbacusComponent):
