...
from slp_abacus.slp_abacus.map.mapping import AttackSurface
from slp_abacus.slp_abacus.map.abacus_mapper import trustzone_to_otm
from slp_abacus.slp_abacus.matcher import ComponentsAndSGsMatcher
from slp_abacus.slp_abacus.objects.abacus_objects import AbacusOTM, AbacusComponent, SecurityGroupCIDR, \
...
from slp_abacus.slp_abacus.transformers.dataflow.strategies.dataflow_creation_strategy import create_dataflow
...
def _create_client(client_id: str, variables: Dict, security_group_cidr: SecurityGroupCIDR,
                   attack_surface_configuration: AttackSurface) -> AbacusComponent:
...
class AttackSurfaceCalculator:
    ...
    @inject
    def __init__(self, otm: AbacusOTM, graph: DiGraph, attack_surface_configuration: AttackSurface):
        self.otm = otm
        self.graph = graph
        self.attack_surface_configuration: AttackSurface = attack_surface_configuration

        self._clients: List[AbacusComponent] = []
    ...
    def __generate_dataflows(self, security_group_cidrs: List[SecurityGroupCIDR], components: List[AbacusComponent]):
    ...
    def __generate_dataflow_by_cidr_block(self, components: List[AbacusComponent], security_group: SecurityGroupCIDR):
    ...
    def __get_client_by_id(self, client_id: str) -> AbacusComponent:
    ...
    def __is_same_dataflow_to_parent(self,
                                     parent_source_node_component: AbacusComponent,
                                     parent_destination_node_component: AbacusComponent,
                                     parent_name: str,
