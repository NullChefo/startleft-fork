...
from slp_abacus.slp_abacus.transformers.transformer import Transformer


def clone_component_by_parents(component: AbacusComponent, parent_ids: [str]) -> [{}]:
    clones = []
...
def set_component_clones_ids(components: [AbacusComponent]):
    for component in components:
        component.clones_ids = [s.id for s in components if s.id != component.id]


def set_component_index(component: AbacusComponent, index: int):
    component.id = f'{component.id}_{index}'


def set_component_parent(component: AbacusComponent, parent_id: str):
...
class HierarchyCalculator(Transformer):
    ...
    def _find_parent_by_closest_relationship(self, component: AbacusComponent, parent_candidates: []):
